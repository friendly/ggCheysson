...
output: 
  rmarkdown::html_vignette:
    ...
    toc_depth: 2 
vignette: >
  %\VignetteIndexEntry{Getting Started with ggCheysson}
...
```{r scatterplot-sequential, fig.height=5, fig.width=7}
...
```{r scatterplot-category, fig.height=5, fig.width=7}
...
```{r barplot-simple, fig.height=5, fig.width=7}
...
```{r barplot-patterns, fig.height=5, fig.width=7}
...
```{r line-graph, fig.height=5, fig.width=8}
...
```{r area-chart, fig.height=5, fig.width=8}
...
```{r faceted, fig.height=6, fig.width=8}
...
```{r grouped-bars, fig.height=5, fig.width=8}
if (requireNamespace("ggpattern", quietly = TRUE)) {
...
```{r map-style, fig.height=6, fig.width=8}
...
```{r diverging-palette, fig.height=4, fig.width=7}
...
- **25 color palettes** in 4 types (sequential, diverging, grouped, category)
...
For more details, see the [package documentation](https://friendly.github.io/ggCheysson/) and visit David Rumsey's [Albums de Statistique Graphique](https://www.davidrumsey.com/luna/servlet/view/search?q=album+de+statistique) collection.
